...
parse_error::parse_error(std::size_t line, std::size_t column, const char *buffer, std::size_t buffer_length, const std::string &message) : std::runtime_error(create_message(line, column, buffer, buffer_length, message))
{

}

std::string parse_error::create_message(std::size_t line, std::size_t column, const char *buffer, std::size_t buffer_length, const std::string &message)
{
...
void parser::parse_file(const std::string &filename, value &result)
{
...
void parser::parse_string(const std::string &string, value &result)
{
	parser p;
...
parser::parser() : buffer_(nullptr), buffer_position_(0), line_(1), column_(0), root_(nullptr), popped_container_(false), state_(state::normal), surrogate_pair_({ 0, 0 }), unicode_nibble_(0)
{
}
...
void parser::push_array()
{
	...
	if (top().is_object())
	{
		...
		auto inserted_iter = top().emplace_object(std::move(key_.second), value::array().copy()).first;
		inserted = &inserted_iter->second;
		key_.first = false;
		key_.second.clear();
	}
	...
	{
		inserted = &*top().emplace_array(value::array().copy());
	}
	else
	{
		throw std::runtime_error("top of stack not container");
...
void parser::push_object()
{
	...
	if (top().is_object())
	{
		...
		auto inserted_iter = top().emplace_object(std::move(key_.second), value::object().copy()).first;
		inserted = &inserted_iter->second;
		key_.first = false;
		key_.second.clear();
	}
	...
	{
		inserted = &*top().emplace_array(value::object().copy());
	}
	else
	{
		throw std::runtime_error("top of stack not container");
...
void parser::handle_structural_token(char c)
{
	switch (c)
	{
	case '[':
...
parse_error parser::make_error(const std::string &message)
{
...
void parser::parse_next(value &result)
{
	result = value::invalid();
	root_ = &result;
	stack_.push_back(root_);
...
void parser::set_buffer(const char *buffer, std::size_t buffer_length)
{
	buffer_ = buffer;
